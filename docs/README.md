...
The first versioned Role runtime slice now implements Catalog-owned BUSINESS/APPLICATION Roles, immutable activated RoleVersions, normalized typed composition, current `RoleExpansionQuery`, and expansion-change facts. APPLICATION composition is limited to target-scoped Entitlements from the same Application; BUSINESS composition may reference target-scoped Entitlements and APPLICATION Roles. BUSINESS expansion resolves child APPLICATION Roles through their current ACTIVE RoleVersion and preserves ordered RoleVersion paths. The public Catalog HTTP contract is not expanded yet.
