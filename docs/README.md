...
- [`operations/scim-principal-connector.md`](operations/scim-principal-connector.md) — first SCIM 2.0 PRINCIPAL discovery/provisioning adapter, secret-resolution edge, reconciliation coverage and provider-error semantics.
...
The first concrete SCIM 2.0 provider adapter now proves the provider edge for PRINCIPAL discovery plus principal create/update/disable. It keeps provider JSON in the observation/adapter boundary, preserves provider IDs/versions, resolves opaque connector secrets only at execution, normalizes retry/error categories, and reports resumed/bounded discovery as PARTIAL rather than destructive COMPLETE coverage. The remote connector-worker protocol is unchanged; broader connector object classes remain future slices. Local durable SCIM execution is now wired behind an opt-in scheduler: it claims the same Integration-owned ProvisioningTask/ReconciliationRun state under connector-work lease fencing, revalidates Access desired state before provider mutation, executes SCIM HTTP outside authoritative transactions, and commits normalized observations/attempt completion afterward. Remote connector-worker v1 remains an alternative execution placement over the same process state.
