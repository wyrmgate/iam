...
The versioned Role runtime now includes the governed Catalog HTTP surface in addition to the internal expansion engine. BUSINESS/APPLICATION Roles are listable/createable/readable/renameable/retirable; RoleVersions are listable/readable and created as complete typed DRAFT composition snapshots, then move through explicit revision-guarded validate and activate operations. Activation atomically supersedes the prior ACTIVE version and preserves immutable activated content. Role and RoleVersion mutations use causal idempotency, RoleVersion has its own optimistic revision, and both collection families use signed tenant/context-bound cursors. APPLICATION composition remains limited to target-scoped Entitlements from the same Application; BUSINESS composition may reference target-scoped Entitlements and APPLICATION Roles. Access continues to consume only the semantic `RoleExpansionQuery`.

The public Access control-plane slice now exposes authoritative AccessAssignment list/create/read plus explicit suspend/resume/cancel/revoke operations, using optimistic revisions, causal idempotency, signed cursors and default-deny administrative authorization. Public creation is MANUAL provenance and supports both Role and Entitlement targets with the same ANY/SPECIFIC validation as the internal Access domain. EffectiveAccess is exposed only through read/list projection endpoints with current semantic support filtering and normalized RoleVersion support paths; desired state and provider fulfillment remain internal/downstream.

The Access role-expansion slice enables Role-target AccessAssignments using the same authoritative lifecycle/validity semantics as direct assignments. Role-derived EffectiveAccess supports are replaced idempotently when a Role or nested APPLICATION Role expansion changes, with normalized RoleVersion path evidence. ANY supports multi-target roles; SPECIFIC requires exactly one expanded ApplicationTarget matching the governed Principal.
...
The first automatic grant-provisioning planning slice now connects Access desired grant revisions to Integration ProvisioningJob/ProvisioningTask creation. Access emits a data-minimized desired-grant planning fact only when `desired_revision` changes; Integration re-queries the current DesiredGrantState, resolves canonical Principal and provider entitlement targets through semantic/owned state, and creates idempotent ADD_GRANT/REMOVE_GRANT work with the current desired revision. Privilege-increasing ADD planning fails closed on unresolved or ambiguous Principal/provider mapping. Desired absence targets every safely known current/prior technical grant and becomes a no-op when none is known. V22 adds deterministic planner job identity without changing existing manual/runtime job semantics.

The first automatic principal-provisioning planning slice now extends that path to DesiredPrincipalState. Access emits desired-principal revision notifications plus explicit same-revision revalidation triggers after Principal lifecycle changes; Integration re-reads current desired state and plans deterministic UPSERT_PRINCIPAL/DISABLE_PRINCIPAL work. First-time creation currently targets exactly one active `scim-2.principal` binding and uses connector-owned `principalUserNameTemplate` with the bounded `{identityId}` token plus an Identity-owned provisioning profile for display metadata. Successful provider mutation emits a data-minimized internal Integration fact; Identity alone materializes or changes authoritative Principal lifecycle, preserving capability ownership and allowing late stale provider effects to converge through revalidation.
