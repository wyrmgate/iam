...
The available restore window and controls are provider/account state and can change independently of this repository. The Neon console for the actual `wyrmgate-iam-dev` project is the operational source of truth for the active plan, configured recovery window, and available restore controls. Do not encode a billing-plan assumption in application code, repository configuration, or secrets.

Before destructive DEV tests, risky migrations, or data resets:

1. verify that the intended recovery point is inside the current provider recovery window;
2. prefer restoring to a disposable branch or otherwise isolated target when the provider supports that workflow;
3. verify expected schema/data state before using a recovery result operationally;
4. remove disposable recovery objects after verification;
5. record the drill result as operational evidence.

Provider-native recovery improves recovery speed but is not the same as an independently retained logical backup. If DEV requires retention beyond the selected provider plan, cross-provider portability, or protection against provider-account loss, add an external `pg_dump`/`pg_restore` process and protect that backup independently.

The managed DEV activation checklist in [`dev-managed-activation.md`](dev-managed-activation.md) includes the required recovery verification.
...
The CI test is a mechanical restore proof for that standalone path. It is not a substitute for operational restore drills against the active managed DEV provider or realistic production recovery exercises.
...
- For managed Neon DEV, periodically verify the active plan, configured recovery window, and provider restore workflow.
- Exercise a recovery to an isolation-safe target after material provider-plan changes and before risky destructive testing.
